...
public class CalcCapitalSActivity extends AppCompatActivity {
    Button btnCalcular;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_calc_capital_s);
        ...
        btnCalcular.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if(valorMonetario.getText().toString().equals("")){
                    ...
                    float porc = Float.parseFloat(valorPorcentagem.getText().toString());
                    float temp = Float.parseFloat(tempoP.getText().toString());
                    float result = juros / ((porc / 100) * temp);
                    DecimalFormat df = new DecimalFormat("##.##");
                    res.setText("Capital: R$" + df.format(result));
                }
            }
        });
    }
